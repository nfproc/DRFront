...
チェックポイントファイルを使用しない方法を用いた場合，使用する Vivado の
バージョンによっては，ロジックアナライザ（ILA）のアップグレードが必要にな
ることがあります．Generate Bitstream ボタンを押したときに，ILA がロックさ
れていて論理合成できない旨のエラーが出た場合には，以下の手順で ILA をアッ
プグレードしてから，再度お試しください．

- Open Project ボタンでプロジェクトを開く
  （Generate Bitstream ボタンを押してエラーが出たなら，この手順は省略）
- Sources タブから Design Sources → TOP → ILA を右クリックし，
  Upgrade IP をクリック（最後にエラーが出るが無視してよい）
- Vivado を閉じ，改めて Generate Bitstream ボタンを押す
  （または，メニューの Tools → Run Tcl Script で GenerateBitstream.tcl
    を実行してもよい）
